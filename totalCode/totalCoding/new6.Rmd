...
display_names <- c(
  "aframe","Alamofire","ExoPlayer","finagle","fresco",
  "guava","immutable-js","jest","marko","Moya",
  "nightmare","rclone","react","recharts","sqlitebrowser",
  "stf","tensorflow","tesseract","tidb","ZeroNet"
)
project_keys <- paste0("lsm_result", 0:19)

project_choices <- setNames(project_keys, display_names)

# ------------------------- Data Loaders -------------------------

# CSV for Heatmap (uses project_key + ".csv", e.g., "lsm_result0.csv")
load_project_data <- function(project_key) {
  file_path <- paste0(project_key, ".csv")
...
load_pos_json <- function(project_key) {
  candidate1 <- paste0(project_key, ".json")
  candidate2 <- paste0(gsub("result", "results", project_key), ".json")
  json_path <- if (file.exists(candidate1)) candidate1 else if (file.exists(candidate2)) candidate2 else NULL
  if (is.null(json_path)) return(NULL)
  js <- tryCatch(fromJSON(readChar(json_path, file.info(json_path)$size)), error = function(e) NULL)
  if (is.null(js)) return(NULL)

  # Expect js to be a flat key-value numeric list for POS features
  data.frame(
    feature = names(js),
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      h4("Controls"),
      # choices 使用 named vector：显示 display_names，但 value 为 project_keys（内部用）
      selectInput("project_select", "Select Project:", choices = project_choices, selected = project_keys[1]),
...
server <- function(input, output, session) {
  ...
  # Heatmap reactive (input$project_select is the internal project_key like "lsm_result0")
  selected_data <- reactive({
    project_key <- input$project_select
    cache <- proj_cache()
    if (!is.null(cache[[project_key]])) return(cache[[project_key]])
    res <- load_project_data(project_key)
    cache[[project_key]] <- res
    proj_cache(cache)
    res
  })
  ...
  output$heatmap_plot <- renderPlotly({
    ...
    if (is.null(dat) || dat$error) {
      msg <- if (is.null(dat)) "Data not loaded" else (if (is.null(dat$message)) "Data not loaded" else dat$message)
      return(plot_ly() %>% layout(title = msg))
    }
  ...
  output$heatmap_table <- renderDT({
    dat <- selected_data()
    if (is.null(dat) || dat$error) return(datatable(data.frame(Message = if (is.null(dat)) "Data not loaded" else dat$message)))
  ...
  # POS Radar + Bar (load_pos_json expects internal project_key like "lsm_result0")
  ...
  output$pos_table <- renderDT({
    all_json <- lapply(project_keys, load_pos_json)
    all_json <- all_json[!sapply(all_json, is.null)]
    if (length(all_json) == 0) return(datatable(data.frame(Message = "No POS data")))
